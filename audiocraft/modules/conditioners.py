...
class T5Conditioner(TextConditioner):
    ...
    def __init__(self, name: str, output_dim: int, finetune: bool, device: str,
        ...
        self.embedding_manager = EmbeddingManager([self.t5_tokenizer,self.t5], placeholder_strings=["*", "@"], num_vectors_per_token=1)
    ...
    def forward(self, inputs: tp.Dict[str, torch.Tensor], wav_path: str) -> ConditionType:
        mask = inputs['attention_mask']
        with torch.set_grad_enabled(self.finetune), self.autocast:
            # embeds = self.t5(**inputs).last_hidden_state
            embeds = self.t5.encoder.embed_tokens(inputs["input_ids"])
            embeds = self.embedding_manager(inputs["input_ids"], embeds, wav_path)
            embeds = self.t5(inputs_embeds=embeds).last_hidden_state
        #pdb.set_trace()
        # print(self.t5.encoder.embed_tokens(inputs["input_ids"]).size())
...
class ConditioningProvider(nn.Module):
    ...
    def forward(self, tokenized: tp.Dict[str, tp.Any], wav_path: str) -> tp.Dict[str, ConditionType]:
        ...
        for attribute, inputs in tokenized.items():
            #pdb.set_trace()
            condition, mask = self.conditioners[attribute](inputs, wav_path)
