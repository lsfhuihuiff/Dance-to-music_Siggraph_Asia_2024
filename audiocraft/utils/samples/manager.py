...
class SampleManager:
    ...
    def add_sample(self, sample_wav: torch.Tensor, epoch: int, index: int = 0,
                   ...
                   generation_args: tp.Optional[tp.Dict[str, tp.Any]] = None, name = None) -> Sample:
        ...
        """
        # pdb.set_trace()
        sample_id = self._get_sample_id(index, prompt_wav, conditions) + name
        ...
        if ground_truth_wav is not None:
            ground_truth_id = sample_id if reuse_id else self._get_tensor_id(ground_truth_wav.sum(0, keepdim=True))+ name
    ...
    def add_samples(self, samples_wavs: torch.Tensor, epoch: int,
                    ...
                    generation_args: tp.Optional[tp.Dict[str, tp.Any]] = None, index = None, name = None) -> tp.List[Sample]:
        ...
        for idx, wav in enumerate(samples_wavs):
            ...
            samples.append(self.add_sample(wav, epoch, index, conditions, prompt_wav, gt_wav, generation_args, name))
        return samples
