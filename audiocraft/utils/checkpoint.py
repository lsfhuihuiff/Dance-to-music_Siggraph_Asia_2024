...
def _safe_save_checkpoint(state: tp.Any, checkpoint_path: Path, is_sharded: bool = False) -> None:
    ...
    with flashy.utils.write_and_rename(checkpoint_path) as f:
        ...
        poseEncoder = {}
        audioEncoderP = {}
        for name, param in state["best_state"]["model"].items():
            if target_parameters_names[0] in name:
                poseEncoder[name] = param
            elif target_parameters_names[1] in name:
                audioEncoderP[name] = param
        torch.save({
        "poseEncoder": poseEncoder,
        "audioEncoderP": audioEncoderP
        }, f)
