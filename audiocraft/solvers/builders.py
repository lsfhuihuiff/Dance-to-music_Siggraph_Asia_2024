...
def get_optim_parameter_groups(model: nn.Module):
    ...
    params = []
    target_parameters_names = [
        "condition_provider.conditioners.description.embedding_manager.poseEncoder",
        # "condition_provider.conditioners.description.embedding_manager.rhythmEncoder",
        "condition_provider.conditioners.description.embedding_manager.audioEncoder.projector"

    ]

    for name, param in model.named_parameters():
        if (target_parameters_names[0] in name) or (target_parameters_names[1] in name) :
        # if (target_parameters_names[0] in name):
...
def get_optimizer(model, cfg: omegaconf.DictConfig) -> Optimizer:
    ...
    if cfg.optimizer == 'adam':
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=cfg.lr, **cfg.adam)
    elif cfg.optimizer == 'adamw':
        optimizer = torch.optim.AdamW( model.parameters(), lr=cfg.lr, **cfg.adam)
        ...
        optimizer = optim.DAdaptAdam(filter(lambda p: p.requires_grad, model.parameters()), lr=cfg.lr, **cfg.adam)
    else:
