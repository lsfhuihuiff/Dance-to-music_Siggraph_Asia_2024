...
class MusicGenSolver(base.StandardSolver):
    ...
    def _prepare_tokens_and_attributes(
        ...
        wav_path = attributes[0]['text']['meta'].path
        ...
        with self.autocast:
            condition_tensors = self.model.condition_provider(tokenized, wav_path)
    ...
    def run_step(self, idx: int, batch: tp.Tuple[torch.Tensor, tp.List[SegmentWithAttributes]], metrics: dict) -> dict:
        ...
        if self.is_training:
            ...
            if self.cfg.fsdp.use:
                ...
                # this schould always be slower but can be useful
    ...
    def generate_audio(self) -> dict:
        ...
        for batch in lp:
            audio, meta = batch
            name = meta[0].meta.path.split('/')[-1][:-4]
            ...
            if self.cfg.generate.lm.unprompted_samples:
                ...
                sample_manager.add_samples(
                    ...
                    ground_truth_wavs=audio, index=i, generation_args=sample_generation_params, name=name)
            ...
            if self.cfg.generate.lm.prompted_samples:
                ...
                sample_manager.add_samples(
                    ...
                    generation_args=sample_generation_params, name=name)
